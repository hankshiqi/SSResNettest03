...
# 数据集参数
data_folder = r'E:\Pycharmprogram\finalmodel2\Real-ESRGAN\datasets\WUrsSET\GETALLTIF'    # 数据存放路径
...
def main():
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        state = {
            'epoch': epoch,
            'generator': generator.state_dict() if not isinstance(generator,
                                                                  nn.DataParallel) else generator.module.state_dict(),
            'discriminator': discriminator.state_dict() if not isinstance(discriminator,
                                                                          nn.DataParallel) else discriminator.module.state_dict(),
            'optimizer_g': optimizer_g.state_dict(),
            'optimizer_d': optimizer_d.state_dict(),
        }
        torch.save(state, 'results/checkpointForRM1.pth')
